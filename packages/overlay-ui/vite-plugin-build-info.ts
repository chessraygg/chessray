...
function refresh(): void {
  ...
  // ISO 8601 UTC, sliced to "YYYY-MM-DD HH:MM" (no seconds — second-level
  // precision was noise; the date matters more for spotting stale builds).
  const iso = new Date().toISOString();
  const time = `${iso.slice(0, 10)} ${iso.slice(11, 16)}`;
  const content =
