...
### Summary (≤132 chars)
```
Real-time chess board recognition and Stockfish evaluation overlay for any tab — chess sites, streams, videos, screenshots.
```
...
• chess.com, lichess.org and other chess sites
...
```
The on-page overlay is a content script that draws arrows, an eval bar, and a PV preview board on top of the captured board. The user may invoke Chessray on any site that happens to show a chess board (chess sites, video streams, image viewers, PDF readers, screenshots), so the script's match pattern cannot be narrowed to a fixed list of domains without removing the feature on the long tail of sites where it is genuinely useful. No site is read or modified — the overlay is drawn into a top-level container; no page DOM is queried.
```
...
| Screenshots | 1280×800 or 640×400 px | ✓ at least 1, up to 5 | Show Chessray analyzing a real board (chess.com / lichess / a stream). At least one should show the side panel + overlay together. |
